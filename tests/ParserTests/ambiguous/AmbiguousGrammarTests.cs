...
namespace ParserTests.ambiguous
{
    ...
    public class AmbiguousGrammarTests
    {
        ...
        [Fact]
        public void TestDanglingElseAmbiguityWithEbnfChoices()
        {
            var parserInstance = new DanglingElseParserWithEbnfChoices();
            var builder = new ParserBuilder<DanglingElseToken, string>();
            var buildResult = builder.BuildParser(parserInstance, ParserType.EBNF_LL_RECURSIVE_DESCENT, "programZero");
            Check.That(buildResult).IsOk();
...
a := 4";
            ...
            var parseResult = parser.Parse(source, "programZero");
            Check.That(parseResult).IsOkParsing(checkIfResultisNull:false);
            Check.That(parseResult.IsAmbiguous).IsTrue(); 
            Check.That(parseResult.Forest).IsNotNull();
            Check.That(parseResult.Forest.Count).IsEqualTo(2); 
            var allResults = parseResult.VisitAllTrees(parser.Visitor); 
            Check.That(allResults).CountIs(2); 
            Check.That(allResults).Contains( "if(x==1,if(y==2,a:=3),a:=4)", "if(x==1,if(y==2,a:=3,a:=4))" );
            
            parseResult = parser.Parse(source,"programOne");
            Check.That(parseResult).IsOkParsing(checkIfResultisNull:false);
            Check.That(parseResult.IsAmbiguous).IsTrue(); 
            Check.That(parseResult.Forest).IsNotNull();
            Check.That(parseResult.Forest.Count).IsEqualTo(2); 
            allResults = parseResult.VisitAllTrees(parser.Visitor); 
            Check.That(allResults).CountIs(2); 
            Check.That(allResults).Contains( "if(x==1,if(y==2,a:=3),a:=4)", "if(x==1,if(y==2,a:=3,a:=4))" );
            
            parseResult = parser.Parse(source,"programOneOrTwo");
            Check.That(parseResult).IsOkParsing(checkIfResultisNull:false);
            Check.That(parseResult.IsAmbiguous).IsTrue(); 
            Check.That(parseResult.Forest).IsNotNull();
            Check.That(parseResult.Forest.Count).IsEqualTo(2); 
        ...
        [Fact]
        public void TestDanglingElseAmbiguityWithRules()
        {
            var parserInstance = new DanglingElseParserWithRules();
            var builder = new ParserBuilder<DanglingElseToken, string>();
            var buildResult = builder.BuildParser(parserInstance, ParserType.EBNF_LL_RECURSIVE_DESCENT, "programZero");
            Check.That(buildResult).IsOk();
