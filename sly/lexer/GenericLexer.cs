    ...
    public class GenericLexer<IN> : ILexer<IN> where IN : struct
    {
        ...
        public LexerResult<IN> Tokenize(ReadOnlyMemory<char> memorySource)
        {
            ...
            var ignored = r.IgnoredTokens.Select(x =>
                {
                    var ignoredToken = new Token<IN>(default(IN), x.SpanValue, x.Position, x.IsComment,
                        x.CommentType, x.Channel);
                    ignoredToken.IsEOL = x.IsEOL;
                    ignoredToken.IsWhiteSpace = x.IsWhiteSpace;
                    ignoredToken.IsComment = x.IsComment;
                    return ignoredToken;
                }
                ).ToList();
            ...
            while (r.IsSuccess)
            {
                ...
                if (SubParserCallBacks.TryGetValue(transcoded.TokenID, out var subparsercallback))
                {
                    ...
                    else
                    {
                        ...
                        {
                                x.Column += (x.Line == 0 ? transcoded.Position.Column : 0);
                                x.Line += transcoded.Position.Line;
                                var message = x.ErrorMessage;
                                var error = new LexicalError(x.Line, x.Column, x.ErrorMessage);
                                return error;
                        }).ToList();
