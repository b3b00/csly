...
namespace sly.parser.generator
{
    ...
    internal class EBNFParserBuilder<IN, OUT> : ParserBuilder<IN, OUT> where IN : struct, Enum
    {
            ...
            LexerPostProcess<IN> lexerPostProcess = null)
        {
            ...
            var grammarParser = builder.BuildParser(ruleparser, RuleParserType.ParserType, "rule").Result;
        ...
        public override ISyntaxParser<IN, OUT> BuildSyntaxParser(ParserConfiguration<IN, OUT> conf,
            ...
            while (stillSetting)
            {
                ...
                foreach (var nt in conf.NonTerminals)
                {
                    foreach (var rule in nt.Value.Rules)
                    {
                        ...
                        {
                            if (clause is NonTerminalClause<IN, OUT> nonTerminalClause)
                            {
